...
MsTimer2:: set ( 5 , to_Timer ) ; MsTimer2:: start ( ) ;
  // clock.setDateTime(__DATE__, __TIME__); // Встановлюємо час на годиннику, ґрунтуючись на часі компіляції скетчу 
...
void loop ( ) {  
  DateTime = clock. getDateTime ( ) ; // Опитування часу 
...
void segment ( ) { 
  switch ( segm ) {                                                                  
    ...
    case  5 : ch ( 2 , 0 ) ; ch ( 3 , 1 ) ; ch ( 4 , 0 ) ; ch ( 5 , 0 ) ; ch ( 6 , 1 ) ; ch ( 7 , 0 ) ;( 8 , 0 ) ; break ; // 5 
...
void anod ( ) { 
  switch ( an ) {                                                              
    case  0 : ch ( 9 , 0 ) ; ch ( 10 , 1 ) ; ch ( 11 , 1 ) ;( 12 , 1 ) ; break ;  
